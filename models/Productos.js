...
module.exports = class Productos {
    ...
    async update(objActualizado){
        ...
        const index = objs.findIndex(o => o.id == objActualizado.id)

        console.log('index: '+index);

        if (index == -1) {
            throw new Error(`Error al actualizar: no se encontró el id ${objActualizado.id}`)
        }
        ...
        try {
            await fs.writeFile(this.ruta, JSON.stringify(objs, null, 2))
            console.log('objeto actualizado'+ JSON.stringify(objs));
        } catch (error) {
